...
-- Table `Optica`.`costat`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `Optica`.`costat` (
  `id_costat` INT NOT NULL AUTO_INCREMENT,
  `costat` VARCHAR(10) NOT NULL,
  `graduacio` FLOAT NOT NULL,
  `id_color` INT NOT NULL,
  PRIMARY KEY (`id_costat`),
...
CREATE TABLE IF NOT EXISTS `Optica`.`caracteristiques` (
  ...
  `id_costat` INT NOT NULL,
  PRIMARY KEY (`id_caracteristiques`),
  INDEX `id_costat_idx` (`id_costat` ASC) VISIBLE,
  INDEX `id_montura_idx` (`id_montura` ASC) VISIBLE,
  CONSTRAINT `id_costat`
    FOREIGN KEY (`id_costat`)
    REFERENCES `Optica`.`costat` (`id_costat`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
